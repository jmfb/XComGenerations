...
namespace XCom.Modals
{
	public class SelectCraftWeapon : Screen
	{
		...
		public SelectCraftWeapon(Craft craft, int weaponSlot)
		{
			...
			AddControl(
				new Border(20, 50, 220, 160, ColorScheme.Purple, Backgrounds.EquipCraft, 10)
			);
			AddControl(
				new Label(28, Label.Center, "Select Armament", Font.Large, ColorScheme.Purple)
			);
			...
			AddControl(
				new ListView<CraftWeaponType>(
					68,
					58,
					10,
					AvailableCraftWeapons,
					ColorScheme.Blue,
					Palette.GetPalette(10).GetColor(230),
					OnSelectCraftWeapon
				)
					...
					.AddColumn(
						50,
						Alignment.Left,
						item => GetAvailableQuantity(item).FormatNumber()
					)
					.AddColumn(
						36,
						Alignment.Left,
						item => GetAvailableAmmo(item)?.FormatNumber() ?? "N.A."
					)
			);
			AddControl(
				new Button(156, 90, 140, 16, "CANCEL", ColorScheme.Purple, Font.Normal, EndModal)
			);
		}
		...
		private void OnSelectCraftWeapon(CraftWeaponType weapon)
		{
			...
			if (weaponSlot < craft.Weapons.Count)
			{
				...
				if (oldWeaponMetadata.Ammo != null)
					stores.Add(
						oldWeaponMetadata.Ammo.Value,
						oldWeapon.Ammunition / oldWeaponMetadata.RoundsInAmmo
					);
