...
namespace XCom.Modals
{
	public class EditResearchProject : Screen
	{
		...
		public EditResearchProject(ResearchProject research)
		{
			...
			AddControl(new DynamicLabel(60, 164, () => selectedBase.GetScientistsAvailable().FormatNumber(), Font.Normal, ColorScheme.White));
			AddControl(new Label(70, 61, "LABORATORY SPACE AVAILABLE>", Font.Normal, ColorScheme.DarkYellow));
			AddControl(new DynamicLabel(70, 196, () => selectedBase.GetLaboratorySpaceAvailable().FormatNumber(), Font.Normal, ColorScheme.White));
			...
			AddControl(new Repeater(100, 195, 13, 14, "U", ColorScheme.DarkYellow, Font.Arrow, OnIncrease, 100));
			AddControl(new Label(120, 85, "Decrease", Font.Large, ColorScheme.DarkYellow));
			AddControl(new Repeater(120, 195, 13, 14, "D", ColorScheme.DarkYellow, Font.Arrow, OnDecrease, 100));
		...
		{
			var selectedBase = GameState.SelectedBase;
			if (selectedBase.GetScientistsAvailable() > 0 && selectedBase.GetLaboratorySpaceAvailable() > 0)
