...
namespace XCom.Modals
{
	public class NotEnoughStoreSpace : Screen
	{
		public NotEnoughStoreSpace(ColorScheme colorScheme, byte[] background, int palette = 6)
		{
			...
			AddControl(
				new Label(75, Label.Center, "NOT ENOUGH STORE SPACE!", Font.Large, colorScheme)
			);
			AddControl(
				new Label(
					90,
					Label.Center,
					"Build a new store facility or transfer existing",
					Font.Normal,
					colorScheme
				)
			);
			AddControl(
				new Label(100, Label.Center, "stores to other bases.", Font.Normal, colorScheme)
			);
