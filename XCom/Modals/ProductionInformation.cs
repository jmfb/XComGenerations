		...
		public ProductionInformation(ManufactureType project, Action<ManufactureProject> action)
		{
			...
			AddControl(
				new Label(
					50,
					16,
					metadata.HoursToProduce.FormatNumber() + " Engineer hours to produce one unit",
					Font.Normal,
					ColorScheme.Blue
				)
			);
			...
			AddControl(
				new Label(60, 87, metadata.Cost.FormatNumber(), Font.Normal, ColorScheme.White)
			);
			...
			AddControl(
				new Label(
					70,
					112,
					metadata.SpaceRequired.FormatNumber(),
					Font.Normal,
					ColorScheme.White
				)
			);
			...
			AddControl(
				new Button(155, 16, 135, 16, "CANCEL", ColorScheme.Blue, Font.Normal, EndModal)
			);
			if (
				production.CanProduce(GameState.SelectedBase)
				&& metadata.SpaceRequired <= GameState.SelectedBase.WorkshopSpaceAvailable
			)
				AddControl(
					new Button(
						155,
			...
			foreach (var storeItem in specialMaterials)
			{
				AddControl(
					new Label(
						nextTop,
						30,
						storeItem.ItemType.Metadata().Name,
						Font.Normal,
						ColorScheme.Blue
					)
				);
				...
				AddControl(
					new Label(
						nextTop,
						245,
						GameState.SelectedBase.Stores[storeItem.ItemType].FormatNumber(),
						Font.Normal,
						ColorScheme.White
					)
				);
