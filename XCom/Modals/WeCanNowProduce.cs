...
namespace XCom.Modals
{
	public class WeCanNowProduce : Screen
	{
		public WeCanNowProduce(IEnumerable<ManufactureType> newManufactureTypes)
		{
			...
			AddControl(
				new Label(20, Label.Center, "We can now produce", Font.Large, ColorScheme.Green)
			);
			var nextTop = 56;
			foreach (var newManufactureType in newManufactureTypes)
			{
				AddControl(
					new Label(
						nextTop,
						Label.Center,
						newManufactureType.Metadata().Name,
						Font.Large,
			...
			AddControl(
				new Button(148, 80, 160, 14, "OK", ColorScheme.Green, Font.Normal, EndModal)
			);
			AddControl(
				new Button(
					164,
