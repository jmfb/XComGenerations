...
public class TopicView : Screen
{
	...
	private void AddTopicControls(ArmorType armor)
	{
		...
		foreach (var stat in stats)
		{
			...
			AddControl(
				new Label(top, 275, stat.Item2.FormatNumber(), Font.Normal, ColorScheme.LightBlue)
			);
		}
	}
	...
	private void AddTopicControls(WeaponType weapon)
	{
		...
		AddControl(
			new Item(4 + 8 * (3 - metadata.Height), 158 + 8 * (2 - metadata.Width), metadata.Image)
		);
		AddControl(new Label(24, 5, metadata.Name, Font.Large, ColorScheme.White));
	...
	private void AddTopicControls(FacilityType facility)
	{
		...
		else
		{
			...
			AddControl(
				new Picture(30 + metadata.RowOffset, 248 + metadata.ColumnOffset, metadata.Image)
			);
		}
