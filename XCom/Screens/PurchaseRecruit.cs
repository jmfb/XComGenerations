...
{
	private readonly Dictionary<ItemType, int> itemsToPurchase = Enum.GetValues(typeof(ItemType))
		.Cast<ItemType>()
	...
	public PurchaseRecruit()
	{
		...
		AddControl(
			new Label(8, Label.Center, "Purchase/Hire Personnel", Font.Large, ColorScheme.Blue)
		);
	...
	private int TotalItemSpaceRequired =>
		itemsToPurchase.Sum(item => item.Key.Metadata().StorageSpace * item.Value);
	private int TotalHangarSpaceRequired =>
		itemsToPurchase.Sum(item => item.Key.Metadata().HangarSpace * item.Value);
	private int TotalLivingSpaceRequired =>
		itemsToPurchase.Sum(item => item.Key.Metadata().LivingSpace * item.Value);
	...
	private void OnPurchaseItem(ItemType item)
	{
		...
		if (
			TotalLivingSpaceRequired + metadata.LivingSpace
			> GameState.SelectedBase.LivingSpaceAvailable
		)
		{
			AbortPurchaseItem();
		...
		if (
			TotalHangarSpaceRequired + metadata.HangarSpace
			> GameState.SelectedBase.HangarSpaceAvailable
		)
		{
			AbortPurchaseItem();
			new NoFreeHangars(ColorScheme.LightMagenta, Backgrounds.Funds, "PURCHASE").DoModal(
				this
			);
			return;
		}
		...
		if (
			TotalItemSpaceRequired + metadata.StorageSpace
			> GameState.SelectedBase.ItemSpaceAvailable
		)
		{
			AbortPurchaseItem();
	...
	private static void PurchaseSoldiers(int count, int purchaseHours)
	{
		foreach (var index in Enumerable.Range(0, count))
			GameState.SelectedBase.TransferredSoldiers.Add(
	...
	{
		foreach (var index in Enumerable.Range(0, count))
			GameState.SelectedBase.TransferredCrafts.Add(
				new TransferItem<Craft>
				{
					Item = Craft.CreateNew(
						CraftType.Skyranger,
						GameState.Current.Data.NextSkyrangerNumber++
					),
					HoursRemaining = purchaseHours,
				}
			);
			...
			GameState.SelectedBase.TransferredCrafts.Add(
				new TransferItem<Craft>
				{
					Item = Craft.CreateNew(
						...
						GameState.Current.Data.NextInterceptorNumber++
					),
					HoursRemaining = purchaseHours,
				}
			);
	...
	private static void PurchaseItems(
		KeyValuePair<ItemType, int> item,
		int count,
		int purchaseHours
	)
	{
		GameState.SelectedBase.TransferredStores.Add(
