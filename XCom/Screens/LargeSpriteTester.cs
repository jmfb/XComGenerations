...
public class LargeSpriteTester : Screen
{
	private readonly Stopwatch stopwatch = new();
	private bool animating;
	private int frame;
	private int spriteIndex;
	private Dictionary<Direction, LargeSprite> Sprites => LargeSprite.All[spriteIndex];

	public LargeSpriteTester()
	{
		AddControl(
			new Button(
				180,
	...
	public override void Render(GraphicsBuffer buffer)
	{
		...
		{
			Sprites[Direction.North].Render(buffer, row1, column1);
			Sprites[Direction.NorthEast].Render(buffer, row1, column3);
			Sprites[Direction.East].Render(buffer, row2, column2);
			Sprites[Direction.SouthEast].Render(buffer, row2, column4);

			Sprites[Direction.South].Render(buffer, row3, column1);
			Sprites[Direction.SouthWest].Render(buffer, row3, column3);
			Sprites[Direction.West].Render(buffer, row4, column2);
			Sprites[Direction.NorthWest].Render(buffer, row4, column4);
		}

		if (animating)
			Font.Normal.DrawString(buffer, 100, 0, $"Frame {frame}", ColorScheme.White);
		else
			Font.Normal.DrawString(buffer, 100, 0, "Static", ColorScheme.White);
	}
}
