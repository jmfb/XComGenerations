...
public class LoadGame : Screen
{
	...
	public LoadGame(Screen returnToScreen)
	{
		...
		AddControl(
			new Label(8, Label.Center, "Select game to load", Font.Large, theme.HeaderScheme)
		);
		...
		foreach (var gameId in Enumerable.Range(1, 10))
		{
			...
			if (GameState.GameDataExists(gameId))
			{
				...
				AddControl(
					new ExtendedLabel(topRow, 36, 159, data.Name, Font.Normal, theme.TextScheme)
				);
				AddControl(
					new Label(
						topRow,
						195,
						data.Time.ToString("H:mm"),
						Font.Normal,
						theme.TextScheme
					)
				);
				AddControl(
					new Label(
						topRow,
						225,
						data.Time.Day.FormatOrdinal(),
						Font.Normal,
						theme.TextScheme
					)
				);
				AddControl(
					new Label(topRow, 255, data.Time.ToString("MMM"), Font.Normal, theme.TextScheme)
				);
				AddControl(
					new Label(
						topRow,
						285,
						data.Time.ToString("yyyy"),
						Font.Normal,
						theme.TextScheme
					)
				);
	...
	private Theme CurrentTheme =>
		returnToScreen is Battlescape.Battlescape ? battlescapeTheme : geoscapeTheme;
}
