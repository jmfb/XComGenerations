...
namespace XCom.Screens
{
	public class Manufacture : Screen
	{
		...
		public Manufacture(Screen returnToScreen)
		{
			...
			AddControl(
				new Label(8, Label.Center, "CURRENT PRODUCTION", Font.Large, ColorScheme.Purple)
			);
			...
			AddControl(
				new DynamicLabel(
					24,
					249,
					() => selectedBase.EngineersAllocated.FormatNumber(),
					Font.Normal,
					ColorScheme.White
				)
			);
			AddControl(
				new Label(34, 8, "Workshop Space Available>", Font.Normal, ColorScheme.Purple)
			);
			AddControl(
				new DynamicLabel(
					34,
			...
			AddControl(
				new ListView<ManufactureProject>(
					80,
					8,
					11,
					selectedBase.ManufactureProjects,
					ColorScheme.Blue,
					Palette.GetPalette(1).GetColor(230),
					OnSelectManufactureProject
				)
					.AddColumn(1, Alignment.Left, project => "")
					.AddColumn(
						105,
						Alignment.Left,
						project => project.ManufactureType.Metadata().Name
					)
					.AddColumn(
						39,
						Alignment.Left,
						project => project.EngineersAllocated.FormatNumber()
					)
					...
					.AddColumn(
						47,
						Alignment.Left,
						project => $"${project.ManufactureType.Metadata().Cost.FormatNumber()}"
					)
