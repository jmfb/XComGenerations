...
namespace XCom.Screens
{
	public class MonthlyCosts : Screen
	{
		public MonthlyCosts()
		{
			...
			AddControl(
				new Label(12, Label.Center, "Monthly Costs", Font.Large, ColorScheme.LightMagenta)
			);
			...
			AddControl(
				new Label(
					120,
					250,
					$"${selectedBase.TotalMaintenance.FormatNumber()}",
					Font.Normal,
					ColorScheme.Blue
				)
			);
			...
			AddControl(
				new Label(
					136,
					250,
					$"${selectedBase.TotalMonthlyCost.FormatNumber()}",
					Font.Normal,
					ColorScheme.White
				)
			);
			...
			AddControl(
				new Button(176, 10, 300, 16, "OK", ColorScheme.LightMagenta, Font.Normal, OnOk)
			);
		}
		...
		private void CreateCostRow(int topRow, ItemType itemType, int quantity)
		{
			...
			AddControl(
				new ExtendedLabel(topRow, 10, 125, metadata.Name, Font.Normal, ColorScheme.Blue)
			);
			AddControl(
				new ExtendedLabel(
					topRow,
					135,
					70,
					$"${metadata.MonthlyCost.FormatNumber()}",
					Font.Normal,
					ColorScheme.Blue
				)
			);
			...
			AddControl(
				new Label(
					topRow,
					250,
					$"${(metadata.MonthlyCost * quantity).FormatNumber()}",
					Font.Normal,
					ColorScheme.Blue
				)
			);
