...
public class ViewSoldierStatistics : Screen
{
	...
	public ViewSoldierStatistics(Battle battle, BattleSoldier soldier)
	{
		...
		AddRow(
			101,
			"FIRING ACCURACY",
			soldier.FiringAccuracy,
			soldier.Soldier.Statistics.FiringAccuracy,
			128
		);
		AddRow(
			111,
			"THROWING ACCURACY",
			soldier.ThrowingAccuracy,
			soldier.Soldier.Statistics.ThrowingAccuracy,
			160
		);
		...
		if (soldier.Soldier.HasPsiSkill)
		{
			AddRow(
				131,
				"PSIONIC STRENGTH",
				soldier.PsionicStrength,
				soldier.Soldier.Statistics.PsionicStrength,
				176
			);
			AddRow(
				141,
				...
				soldier.Soldier.Statistics.PsionicSkill,
				176
			);
		}
	...
	)
	{
		...
		AddControl(
			new Bar(topRow + 1, 170, maxValue, 5, value, borderColor ?? (fillColor + 7), fillColor)
		);
	}
