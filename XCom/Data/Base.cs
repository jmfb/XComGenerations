...
namespace XCom.Data
{
	public class Base
	{
		...
		[JsonIgnore]
		private int TransferredEngineerCount =>
			TransferredStores
				.SingleOrDefault(transfer => transfer.Item.ItemType == ItemType.Engineer)
				?.Item.Count ?? 0;

		[JsonIgnore]
		private int TransferredScientistCount =>
			TransferredStores
				.SingleOrDefault(transfer => transfer.Item.ItemType == ItemType.Scientist)
				?.Item.Count ?? 0;
		...
		public int CountFacilities(FacilityType facilityType)
		{
			return Facilities.Count(facility =>
				facility.DaysUntilConstructionComplete == 0 && facility.FacilityType == facilityType
			);
		}
		...
		public IEnumerable<Craft> ActiveInterceptors =>
			Crafts.Where(craft => craft.Status == CraftStatus.Out);

		private static int GetMonthlyCost(ItemType itemType, int count) =>
			itemType.Metadata().MonthlyCost * count;

		private int MonthlySkyrangerCost => GetMonthlyCost(ItemType.Skyranger, TotalSkyrangerCount);
		private int MonthlyInterceptorCost =>
			GetMonthlyCost(ItemType.Interceptor, TotalInterceptorCount);
		...
		private int TransferredItemSpaceRequired =>
			TransferredStores.Sum(transfer => transfer.Item.TotalItemSpaceRequired);
		private int TotalItemSpaceRequired =>
			Stores.TotalItemSpaceRequired + TransferredItemSpaceRequired;
		...
		[JsonIgnore]
		public int ScientistsAllocated =>
			ResearchProjects.Sum(research => research.ScientistsAllocated);
		...
		[JsonIgnore]
		public int EngineersAllocated =>
			ManufactureProjects.Sum(project => project.EngineersAllocated);

		[JsonIgnore]
		public int EngineersAvailable => EngineerCount - EngineersAllocated;

		private int WorkshopSpaceUsed =>
			ManufactureProjects.Sum(project => project.ManufactureType.Metadata().SpaceRequired);

		[JsonIgnore]
		public int WorkshopSpaceAvailable =>
			TotalWorkshopSpace - EngineersAllocated - WorkshopSpaceUsed;
		...
		public int TotalCraftCount =>
			Crafts.Count + TransferredCrafts.Count + CraftUnderConstruction;
		...
		private static IEnumerable<ResearchType> ExhaustedResearch =>
			GameState.Current.Data.CompletedResearch.Where(research =>
				research.Metadata().IsExhausted(GameState.Current.Data.CompletedResearch)
			);
		private IEnumerable<ResearchType> RemainingResearchProjects =>
			AllResearchProjects.Except(ExhaustedResearch).Except(ActiveResearchProjects);

		[JsonIgnore]
		public List<ResearchType> AvailableResearchProjects =>
			RemainingResearchProjects
				.Where(research =>
					research
						.Metadata()
						.IsRequiredResearchCompleted(GameState.Current.Data.CompletedResearch)
				)
		...
		private IEnumerable<ManufactureType> RemainingManufactureProjects =>
			AllManufactureProjects.Except(ActiveManufactureProjects);

		[JsonIgnore]
		public List<ManufactureType> AvailableManufactureProjects =>
			RemainingManufactureProjects
				.Where(project =>
					project
						.Metadata()
						.IsRequiredResearchCompleted(GameState.Current.Data.CompletedResearch)
		...
		{
			return Facilities.FirstOrDefault(facility =>
				facility.IsAt(row, column, allowUnderConstruction)
			);
		}
		...
		public bool CanDismantleFacility(Facility facility)
		{
			...
			facilityConnectivity.Enter(
				Facilities.Single(accessLift => accessLift.FacilityType == FacilityType.AccessLift)
			);
		...
		private class FacilityConnectivity
		{
			...
			private readonly Status[][] facilities = Enumerable
				.Range(0, rowCount)
			...
			private void MarkNeighbor(int row, int column)
			{
				...
				if (neighbor != null && facilities[row][column] == Status.None)
					MarkFacility(
						neighbor,
						neighbor.DaysUntilConstructionComplete == 0
							? Status.Reachable
							: Status.Visited
					);
			}
