...
namespace XCom.Data
{
	...
	public static class ItemTypeExtensions
	{
		...
		private static ItemMetadata StoreItem(
			string name,
			int purchaseHours,
			int cost,
			int salePrice,
		...
		private static ItemMetadata StoreEquipment(
			string name,
			int purchaseHours,
			int cost,
			int salePrice,
		...
		private static ItemMetadata StoreAmmo(
			string name,
			int purchaseHours,
			int cost,
			int salePrice,
		...
		private static ItemMetadata StoreTank(
			string name,
			int purchaseHours,
			int cost,
			int salePrice
		...
		private static ItemMetadata OtherEquipment(
			string name,
			int salePrice,
			int storageSpace,
		...
		private static ItemMetadata OtherAmmo(
			string name,
			int salePrice,
			int storageSpace,
			ItemType weapon
		...
		{
			return new ItemMetadata { Name = name };
		}
		...
		>
		{
			...
			{
				ItemType.PistolClip,
				StoreAmmo("Pistol Clip", 24, 70, 52, 10, ItemType.Pistol, Items.PistolClip)
			},
			{ ItemType.Rifle, StoreEquipment("Rifle", 24, 3000, 2250, 20, Items.Rifle) },
			{
				ItemType.RifleClip,
				StoreAmmo("Rifle Clip", 24, 200, 150, 10, ItemType.Rifle, Items.RifleClip)
			},
			{
				ItemType.HeavyCannon,
				StoreEquipment("Heavy Cannon", 24, 6400, 4800, 30, Items.HeavyCannon)
			},
			{
				ItemType.HcApAmmo,
				StoreAmmo("HC-AP Ammo", 24, 300, 225, 10, ItemType.HeavyCannon, Items.HcApAmmo)
			},
			{
				ItemType.HcHeAmmo,
				StoreAmmo("HC-HE Ammo", 24, 500, 275, 10, ItemType.HeavyCannon, Items.HcHeAmmo)
			},
			{
				ItemType.HcIAmmo,
				StoreAmmo("HC-I Ammo", 24, 400, 300, 10, ItemType.HeavyCannon, Items.HcIAmmo)
			},
			{
				ItemType.AutoCannon,
				StoreEquipment("Auto-Cannon", 24, 13500, 10125, 30, Items.AutoCannon)
			},
			{
				ItemType.AcApAmmo,
				StoreAmmo("AC-AP Ammo", 24, 500, 400, 10, ItemType.AutoCannon, Items.AcApAmmo)
			},
			{
				ItemType.AcHeAmmo,
				StoreAmmo("AC-HE Ammo", 24, 700, 560, 10, ItemType.AutoCannon, Items.AcHeAmmo)
			},
			{
				ItemType.AcIAmmo,
				StoreAmmo("AC-I Ammo", 24, 650, 520, 10, ItemType.AutoCannon, Items.AcIAmmo)
			},
			{
				ItemType.RocketLauncher,
				StoreEquipment("Rocket Launcher", 24, 4000, 3000, 40, Items.RocketLauncher)
			},
			{
				ItemType.SmallRocket,
				StoreAmmo(
					"Small Rocket",
					24,
					600,
			...
			},
			{
				...
				StoreAmmo(
					...
					20,
					ItemType.RocketLauncher,
					Items.IncendiaryRocket
				)
			},
			{ ItemType.Grenade, StoreEquipment("Grenade", 24, 300, 240, 10, Items.Grenade) },
			{
				ItemType.SmokeGrenade,
				StoreEquipment("Smoke Grenade", 24, 150, 120, 10, Items.SmokeGrenade)
			},
			{
				ItemType.ProximityGrenade,
				StoreEquipment("Proximity Grenade", 24, 500, 400, 10, Items.ProximityGrenade)
			},
			{
				ItemType.HighExplosive,
				StoreEquipment("High Explosive", 24, 1500, 1200, 20, Items.HighExplosive)
			},
			{ ItemType.StunRod, StoreEquipment("Stun Rod", 24, 1260, 945, 10, Items.StunRod) },
			{
				ItemType.ElectroFlare,
				StoreEquipment("Electro-flare", 24, 60, 40, 10, Items.ElectroFlare)
			},
			...
			{
				ItemType.PlasmaPistol,
				OtherEquipment(
					"Plasma Pistol",
					84000,
					10,
					ResearchType.PlasmaPistol,
					ResearchType.PlasmaPistolClip
				)
			},
			{
				ItemType.PlasmaPistolClip,
				OtherAmmo("Plasma Pistol Clip", 4440, 10, ItemType.PlasmaPistol)
			},
			{
				...
				OtherEquipment(
					...
					126500,
					20,
					ResearchType.PlasmaRifle,
					ResearchType.PlasmaRifleClip
				)
			},
			{
				ItemType.PlasmaRifleClip,
				OtherAmmo("Plasma Rifle Clip", 6290, 10, ItemType.PlasmaRifle)
			},
			{
				ItemType.HeavyPlasma,
				OtherEquipment(
					"Heavy Plasma",
					171600,
					20,
					ResearchType.HeavyPlasma,
					ResearchType.HeavyPlasmaClip
				)
			},
			{
				ItemType.HeavyPlasmaClip,
				OtherAmmo("Heavy Plasma Clip", 9590, 30, ItemType.HeavyPlasma)
			},
			{
				ItemType.SmallLauncher,
				OtherEquipment(
					"Small Launcher",
					120000,
					20,
					ResearchType.SmallLauncher,
					ResearchType.StunBomb
				)
			},
			{ ItemType.StunBomb, OtherAmmo("Stun Bomb", 15200, 10, ItemType.SmallLauncher) },
			{
				ItemType.BlasterLauncher,
				OtherEquipment(
					"Blaster Launcher",
					144000,
					30,
					ResearchType.BlasterLauncher,
					ResearchType.BlasterBomb
				)
			},
			{
				ItemType.BlasterBomb,
				OtherAmmo("Blaster Bomb", 17028, 20, ItemType.BlasterLauncher)
			},
			{
				ItemType.AlienGrenade,
				OtherEquipment("Alien Grenade", 14850, 10, ResearchType.AlienGrenade)
			},
			{
				ItemType.MindProbe,
				OtherEquipment("Mind Probe", 304000, 10, ResearchType.MindProbe)
			},
