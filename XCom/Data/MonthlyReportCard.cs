...
namespace XCom.Data
{
	public class MonthlyReportCard
	{
		...
		public static MonthlyReportCard ScoreMonth(DateTime month)
		{
			...
			var happyCountries = data
				.Countries.Where(country => country.Satisfaction == CountrySatisfaction.Happy)
				.ToList();
			var unhappyCountries = data
				.Countries.Where(country => country.Satisfaction == CountrySatisfaction.Unhappy)
				.ToList();
			var countriesThatSignedPactsWithAliens = data
				.Countries.Where(country =>
					country.Satisfaction == CountrySatisfaction.SignedAlienPact
					&& !country.SignedAlienPact
				)
				.ToList();
		...
		private static string GetCountryList(IReadOnlyList<Country> countries)
		{
			...
			var commaSeparatedCountries = string.Join(
				", ",
				countries.Take(countries.Count - 1).Select(country => country.Name)
			);
			return $"{commaSeparatedCountries} and {countries.Last().Name}";
		}

		private static string PluralizeText(
		...
		private string HappyText =>
			PluralizeText(HappyCountries, singularHappyText, pluralHappyText);

		private const string singularUnhappyText =
		...
		private const string pluralUnhappyText =
			" are unhappy with your ability to deal with alien activity in their territory and have decided to reduce their funding.";
		private string UnhappyText =>
			PluralizeText(UnhappyCountries, singularUnhappyText, pluralUnhappyText);

		private const string singularAlienPactText =
			" has signed a secret pact with unknown alien forces and has withdrawn from the project.";
		private const string pluralAlienPactText =
			" have signed a secret pact with unknown alien forces and have withdrawn from the project.";
		private string AlienPactText =>
			PluralizeText(
				CountriesThatSignedPactsWithAliens,
				singularAlienPactText,
				pluralAlienPactText
			);
